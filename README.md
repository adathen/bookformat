...
- **網頁版**（`docs/` 資料夾，透過 GitHub Pages 發布）：免安裝，開網址、上傳
  ...
  支援兩種來源檔案：
  - **上傳 .docx**：若原始 docx 有手動分頁點（Ctrl+Enter），100% 依照分頁點；
    沒有的話，會用文件實際的頁面尺寸／邊界（從 docx 的 w:pgSz/w:pgMar 讀取），
    在瀏覽器中真實排版整份內容並測量每段高度，據此推算分頁位置——這是真的
    版面測量，不是字數猜測，但字型細節跟 Word 仍有差異，可能需要在網頁上
    手動調整（介面上點一下分隔線即可切換），內容會重新繪製成圖片再組成 PDF。
...
- `js/paginate.js` — 分頁邏輯：有手動分頁點就 100% 依照分頁點；沒有則用
  `RenderPage.buildFlowNode` 把整份文件連續排版，量測每個區塊實際渲染高度，
  依文件真實頁面內容高度推算分頁位置（真實版面測量，不是字數猜測）
